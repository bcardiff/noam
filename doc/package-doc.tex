...
Hay un \jc{FormalismConverter} por formalismo que identifica el formalismo inicial. Éstos se encargan de validar la entrada y realizar la conversión hacia los otros formalismos.
...
Se encuentra la representación de un AF (cualquier autómata finito). Un AF es todo aquel que implementa la interfaz \jc{AF}.
Hay una implementación trivial: \jc{AFND}.

También se encuentra la interfaz \jc{IAutomataBuilder} que es usada para desacoplar la creación de un autómata de la clase que lo representa. Ésto se usa en el parser.
...
crear estados nuevos, para simplificar el desarrollo de los mismo, los autómatas (pero no el parser) soportan estados con nombres de más
...
métodos que correspondan de acuerdo a lo que se esté parseando ej.: agregado de estados, terminales, transiciones, etc. 
...
Cada regla de la gramática retorna una instancia que representa el AST generado por dicha regla.

\subsection{package noam.gr}

Se encuentra la representación de una gramática (\jc{Grammar}). 

También se encuentra la interfaz \jc{IGrammarBuilder} que es usada para desacoplar la creación de una gramática del parser.

\subsection{package noam.gr.algorithms}

Se encuentra la conversión de GR a AF y un algoritmo para normalización de gramáticas para asegurar que solo aparezcan producciones lambda para el símbolo distinguido.
...
métodos que corresponda de acuerdo a lo que se esté parseando ej.: agregado de no terminales, terminales, producciones, etc. 

\subsection{package noam.gr.internal}

Se encuentra una implentación de \jc{IGrammarBuilder} que genera un \jc{Grammar}.
