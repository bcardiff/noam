...
La determinación se genera pre calculando los nuevos estados (que se corresponden con subconjuntos de estados del autómata original). En lugar de generar todos los subconjuntos de estados del autómata original, se parte de la clausura lambda del estado inicial y se recorren solo las transiciones alcanzables a partir de este y de los estados que se van generando. El resto del autómata, es decir, las transiciones y los estados finales se generan a partir de ése cómputo.
...
Durante el algoritmo para completar el autómata se asegura que todos los estados tienen
transiciones por cada terminal del alfabeto. Ésto potencialmente genera un estado trampa.
...
Además de esta transformación, se encuentra en \jc{Normalization} un algoritmo que elimina las
producciones de la forma $A \rightarrow \lambda$ para todo $A$ que no sea el símbolo distinguido y las no
alcanzables. El mismo es compuesto con la salida de \jc{AFToGr} para normalizar la transformación.
Este algoritmo transforma las producciones de la siguiente forma:

\begin{itemize}
\item Para cada producción de la forma $A \rightarrow tB$, si existe una producción $A \rightarrow 
\lambda$, se agrega la producción $A \rightarrow t$.
\item Se eliminan todas las producciones de la forma $A \rightarrow \lambda$ tales que $A$ no es el
símbolo distinguido.
\item Se buscan las producciones de la forma $A \rightarrow tB$ tales que $A$ no es el símbolo distinguido
